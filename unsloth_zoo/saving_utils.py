...
def _merge_lora(W, lora_stats, name):
    ...
    W = W.to("cpu", dtype = torch.float32, non_blocking = True)
    W = W.addmm_(
        lora_stats.lora_B.to("cpu", dtype = torch.float32, non_blocking = True),
        lora_stats.lora_A.to("cpu", dtype = torch.float32, non_blocking = True),
