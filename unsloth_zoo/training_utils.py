...
def unsloth_train(trainer):
    ...
    step = 0
    accumulated_loss = torch.zeros(1, device = "cpu", dtype = torch.float32)[0]
    debug_info = \
        ...
        for epoch in range(num_train_epochs):
            ...
            for j in range(max_iters_per_epoch):
                ...
                for batch in batches:
                    input_ids = batch["input_ids"].pin_memory().to(device = "cpu", non_blocking = True)
                    labels    = batch["labels"]   .pin_memory().to(device = "cpu", non_blocking = True)
