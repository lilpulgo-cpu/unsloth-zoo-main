    ...
    def from_dict(cls, qs_dict: Dict[str, Any], device: torch.device) -> "QuantState":
        ...
        if "nested_absmax" in qs_dict:
            ...
            with torch.autocast(device_type = "cpu", enabled = False):
                offset = torch.tensor(qs_dict["nested_offset"], dtype = torch.float32, device = "cpu")
            state2 = cls(
...
def create_empty_causal_lm(config, dtype = torch.float16):
    ...
    new_model = new_model.to(device = "cpu", dtype = dtype)
    return new_model
pass
...
def convert_vllm_to_huggingface(quant_state_dict, config, dtype = torch.float16):
    ...
    # Override .to("cpu") to disable it otherwise we'll get
    ...
    for kk in range(config.num_hidden_layers):
        for layer_name in layer_names:
            ...
            if f"{layer_name}.weight.quant_state" in quant_state_dict:
                ...
                layer = Linear4bit(0, 0, device = "cpu", bias = has_bias, compute_dtype = compute_dtype, **kwargs)
                ...
                layer = Linear(0, 0, device = "cpu", bias = has_bias)
    ...
    layer = Linear(0, 0, device = "cpu", bias = False)
    layer.in_features  = weight.shape[1]
    layer.out_features = weight.shape[0]
    layer.weight = torch.nn.Parameter(weight, requires_grad = False)
...
def _test_same_model(model, new_model, input_ids):
    ...
    position_ids = torch.arange(input_ids.shape[1], device = "cpu")
    position_ids = position_ids.repeat((1, input_ids.shape[0]))
    rotary_A =     model.model.rotary_emb(A, position_ids)
    new_rotary = new_model.model.rotary_emb.__class__(new_model.config, device = "cpu")
...
def _test_get_vllm_state_dict(
    ...
    input_ids = input_ids["input_ids"].to("cpu", non_blocking = True)
